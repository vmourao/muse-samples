// ==UserScript==
// @name         Connections Cloud Auto-Tagger
// @namespace    https://apps.na.collabserv.com/
// @version      1.0
// @description  Provides the ability to auto-tag (i.e.generate keywords) content using Watson AlchemyAPI.
// @author       Scott Davis
// @match        https://apps.na.collabserv.com/*
// @match        https://apps.ce.collabserv.com/*
// @match        https://apps.ap.collabserv.com/*
// @match        http://tsc.ibmcollabcloud.com/*
...
(function() {
    ...
    function autoTagEvent(event)
    {
        ...
        {
            // Found content, so pass it to the keyword service for processing along with
            // the function to handle them.
            var serviceAPIKey="b9d94c2829e3dd21b509e77bbdafd32a31d64e87";

            // var postData = "apikey=" + serviceAPIKey + "&extract=keywords&outputMode=json&text="+encodeURI(descriptionContent);
            var postData = "extract=keywords&outputMode=json&text="+encodeURI(descriptionContent);
            var sizePostData = descriptionContent.length;

            var watsonURL = "https://gateway-a.watsonplatform.net/calls/text/TextGetCombinedData?apikey="+serviceAPIKey;

            // Send the right type of content-type
            var textType = "text/plain";
            /*
            var isAnIFrame = event.target.getAttribute("contentElemIFrame");
            if ( isAnIFrame !== undefined && isAnIFrame !== null )
            {
                if ( isAnIFrame === "true" )
                {
                    textType = "text/html";
                    watsonURL = "https://gateway-a.watsonplatform.net/calls/html/HTMLGetCombinedData?apikey="+serviceAPIKey;
                }
            }*/
            ...
            GM_xmlhttpRequest({
                ...
                headers: {
                    ...
                    "Content-Length": sizePostData,
                    "Connection": "close"
                },
                synchronous: false,
                onerror: function(response) { alert('Error during XMLHTTPRequest'); },
                ontimeout: function(response) { alert('Timeout during XMLHTTPRequest'); },
                onload: function(response) { processAndInsertTags(response.responseText,event.target.getAttribute("tagInputID")); }
            });
        }
        else
        {
