...
//
// @name         bizCardEnhancements
// @version      0.2
// @description  *** PROTOTYPE CODE *** [script description goes here]
//
...
if(typeof(dojo) != "undefined") {
   require(["dojo/domReady!"], function(){
     ...
     try {
         ...
         dojo.place( 
              '<style>'+
                 '#cardDiv.lotusVCard { box-shadow: 10px 10px 30px gray; }'+
                 '#cardDiv.lotusVCard .lotusPhoto img {  }'+
                 '#javlinDiv, #javlinDiv .photoCard img { box-shadow: 10px 10px 30px gray; } '+
                 '#javlinDiv .personMenu { border:1px solid gray; }'+              
             '</style>', 
              dojo.body(), "first");         
         
      } catch(ex) {
          alert("bizCardEnhancements:"+ex);
      }
   });
}
